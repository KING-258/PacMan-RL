...
Train a single SaveLoadApproximateQAgent model for a specific layout and ghost
type for a fixed number of episodes (default: 4000). Also records per-episode
training scores and summary metrics, saved to JSON.

Outputs:
- Model weights: backend/models/approx-{layout}-{ghost}-{episodes}ep.json
- Training log: backend/outcomes/train/{layout}-{ghost}-{episodes}ep.train.json

Example:
  PACMAN_REPO=/home/king/Pacman-ReinforcementLearning \
  python backend/train_single.py --layout originalClassic --ghost DirectionalGhost \
    --episodes 4000 --num-ghosts 4 --epsilon 0.1 --alpha 0.2 --gamma 0.8
"""
